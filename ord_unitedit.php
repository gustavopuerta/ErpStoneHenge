...
if (!$upd_lock) {
    ...
    if ($_POST['act'] == "save") {
        if($uid==''){
            if (isset($_POST['alastid']) && lastid_check($_POST['alastid'])) {
                ...
                if (!$_SESSION["user"]->db->insert($ssq = "INSERT INTO ord_unit (id_order,
																				 id_application,
																				 id_ord_unit,
																				 status,
																				 id_edge,
					 ...
					 '" . $edit['is_rectangle'] . "','0','" . $edit['slab_nbr'] . "','" . $_SESSION["user"]->getWorkerID() . "',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'" . $_SESSION["user"]->getWorkerID() . "')"))
                {
                    ...
                    $_SESSION["user"]->db->select("exec dbo.ord_unitprice_new ".$oid.",".$uid);
                    $_SESSION["user"]->db->select("exec dbo.ord_orderprice_new ".$oid);
                    if (!$_SESSION["user"]->db->insert($ddc = "INSERT INTO 	 (id_ord_unit,id_slab_frame,sqft_price,slab_price,profit,pickup_profit,m_quality,cutting_risk,severity,waste_sqft,t_price,o_price)
    ...
    if ($_POST['act'] == "delete_u") {
        $sql = "delete from dbo.ord_unit where id_order = ".$oid." and id = ".$edit['id_unit']."";
        if ($_SESSION["user"]->db->select($sql)){
            $error = 'Deleting rectangle failed.';
        }else{
            $_SESSION["user"]->db->select("exec dbo.ord_unitprice_new ".$oid.",".$uid);
            $_SESSION["user"]->db->select("exec dbo.ord_orderprice_new ".$oid);
...
$rsunits = $_SESSION['user']->db->fetchAllArrays();
if (is_array($rsunits))
{
    $rsunits_cnt = count($rsunits);
}
...
$smarty->assign('crunits1',$rsunits);
