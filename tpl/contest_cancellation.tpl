...
<script type="text/javascript" language="javascript">
	...
	function LoadData(){
		...
		if(req){
			...
			url = './contest_response.php?type=3&id_order='+document.getElementById('id_order').value;
	...
	function load_data_text(){
        if (req.readyState == 4) {
            if (req.status == 200){
				...
				document.getElementById('reason_cancel').value=resp;
            }
            else {
                return false;
            }
	...
	function save(){
		...
		if(req){
			...
			url = './contest_response.php?type=2&id_order='+document.getElementById('id_order').value+'&text='+document.getElementById('reason_cancel').value;
			req.open('GET', url, true);
			req.send(null);
		}
    }
...
<br>
<form id="form_cancellation" method="post">
{assign var="table_width" value="350"}
{assign var="table_headertitle" value="Reason for Cancellation:"}
{include file="table_header.tpl"}
<tr>
    <td align="center">
        <textarea id="reason_cancel" rows="3" cols="40" maxlength="200"></textarea>
    </td>
</tr>
<tr>
