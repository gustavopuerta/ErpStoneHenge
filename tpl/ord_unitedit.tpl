...
<script language="javascript">
    ...
    function DeleteUnit(id){
        var r=confirm("Are you sure you want to remove order unit "+id+"?");
        if (r==true)
        {
            document.edit['act'].value = 'delete_u';
            document.edit.submit();
        }
    }
...
<tr >
    ...
    <td rowspan="3"valign="top" width="35%">
        {assign var="table_width" value="450"}
...
{/if}
<tr class="cell_reccolor_lightblue_01a" {if $uid eq ''}style="display:none;"{/if}>
    <td>Honing:</td>
    <td>
        {if $lock_select==''}
...
<tr class="cell_reccolor_lightblue_01b"  {if $uid eq ''}style="display:none;"{/if}>
...
<center>
    ...
    {if $uid==''}
        <input type="button" class="BUTTON_OK" value="Add" onClick="sub(3);">
    {else}
...
<!------------------------------------------------------------------------------------------------>
{if $uid==''}
    {capture assign="table_headertitle"}WO Units{/capture}
    {assign var="table_width" value="450"}
    {include file="table_header.tpl"}
    {assign var="table_link" value=""}
    <tr class="cell_reccolor_lightblue_01a">
        <td width="100"><b>Name</b></td>
        <td width="100"><b>Application</b></td>
        <td width="60"><b>Default Edge</b></td>
        <td width="50"></td>
    </tr>
    {section name=unt loop=$crunits1}
        <tr class="{cycle values="cell_reccolor_lightblue_01a,cell_reccolor_lightblue_01b"}";>
        <td valign="top">{$crunits1[unt].name}</td>
        <td valign="top">{$crunits1[unt].app_name}</td>
        <td valign="top">{$crunits1[unt].default_edge}</td>
        <td valign="top">
            <img src="gfx/trash1.gif" onmouseover="this.src='gfx/trash0.gif'" onClick="DeleteUnit({$crunits1[unt].id});" onmouseout="this.src='gfx/trash1.gif'" alt="Remove '{$crunits1[unt].id}'" border="0" ></a><input type="hidden" name="edit[id_unit]" value="{$crunits1[unt].id}" size="2" />
        </td>
        </tr>
    {/section}
